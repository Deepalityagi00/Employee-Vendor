...
class EmployeeView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not name:
            return Response(
                {"message": "name not passed."}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        return Response(
            {"message": "employee created."}, status=status.HTTP_201_CREATED
        )
...
class ExpensesView(GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not (vendor_code or employee_code):
            return Response(
                {"Details": "Provide Vendor Code or Employee code"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        if vendor_code:
